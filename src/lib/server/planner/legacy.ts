...
export const SETTINGS_DEFAULTS: Omit<PlannerSettings, 'id' | 'created_at' | 'updated_at'> = {
...
export async function ensurePlannerSettings(): Promise<PlannerSettings> {
  const { data, error: queryError } = await supabaseAdmin
...
export async function listInstances(monthKey: string): Promise<TaskInstance[]> {
  const { data, error: queryError } = await supabaseAdmin
    .from('task_instances')
    .select('*')
...
export async function listScheduleBlocksForMonth(monthKey: string): Promise<ScheduleBlock[]> {
  const { data, error: queryError } = await supabaseAdmin
...
export function buildScheduleHealth(tasks: TaskInstance[], blocks: ScheduleBlock[], capacity: CapacitySnapshot): ScheduleHealth {
...
export function buildSoftAssignments(
  tasks: TaskInstance[],
...
export function getDefaultMonthlyWeekKey(
...
export function getTemplateDefaultSyncUpdates(
